...
void generateRandomGraph(const std::string& filename, int vertexCount, int edgeCount)
{
    ...
    // Nagłówek z liczbą wierzchołków i krawędzi
    outFile << vertexCount << " " << edgeCount << "\n";

    // Generuj wszystkie możliwe krawędzie (bez powtórzeń i pętli)
    ...
    // Wymieszaj możliwe krawędzie
    std::shuffle(possibleEdges.begin(), possibleEdges.end(), gen);

    // Zapisz wybrane krawędzie do pliku (każdą tylko raz)
...
void displayMenu()
{
    ...
    std::cout << "10. Display graph\n";
    std::cout << "11. Generate random graph\n";
    std::cout << "0. Exit\n";
    std::cout << "Enter choice: ";
}

void handleUserInput(std::unique_ptr<Graph>& graph)
{
    int choice;
    do
    {
